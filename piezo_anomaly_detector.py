...
"""
PIEZO Anomaly Detection Module
"""
import numpy as np
import json
import csv
...
class PiezoAnomalyDetector:
    """
    Real-time PIEZO anomaly detector using TensorFlow Lite model
    """
    
    def __init__(self, 
                 model_path: str = "unified_piezo_anomaly_detector_quantized.tflite",
                 threshold: float = None,
                 log_format: str = "json",
                 notification_callback: Optional[Callable] = None):
        """
        Initialize the PIEZO anomaly detector
    ...
    def _load_model(self):
        ...
        active_model_path = Path("models/piezo/active_model.json")
        model_loaded_from_config = False
        
        if active_model_path.exists():
            try:
                print(f"[PIEZO Anomaly] Reading active model configuration...")
                ...
                if model_folder:
                    ...
                    model_dir = Path("models/piezo") / model_folder
                    ...
                    if model_file.exists():
                        ...
                        print(f"[PIEZO Anomaly] Using active model: {model_folder}")
                        print(f"[PIEZO Anomaly] Model path: {self.model_path}")
                        print(f"[PIEZO Anomaly] Threshold: {self.threshold}")
                    else:
                        print(f"[PIEZO Anomaly] WARNING: Model file not found in {model_dir}")
                        print(f"[PIEZO Anomaly] Falling back to default model path")
            except Exception as e:
                print(f"[PIEZO Anomaly] Error reading active model config: {e}")
                print(f"[PIEZO Anomaly] Falling back to default model path")
        
        # Fallback: use provided model_path if active config not available
        if not model_loaded_from_config:
            if not self.model_path.exists():
                raise FileNotFoundError(f"PIEZO model not found: {self.model_path}")
            print(f"[PIEZO Anomaly] Using default model path: {self.model_path}")
        
        # Load TFLite model
        print(f"[PIEZO Anomaly] Loading model from: {self.model_path}")
        ...
        print(f"[PIEZO Anomaly] Model loaded successfully. Input shape: {input_shape}")
            ...
            if config_path.exists():
                try:
                    with open(config_path, 'r') as f:
                        ...
                        print(f"[PIEZO Anomaly] Loaded threshold from model config: {self.threshold}")
                except Exception as e:
                    print(f"[PIEZO Anomaly] Error reading model config: {e}")
                    self.threshold = 0.15
            else:
                self.threshold = 0.15  # Default threshold for PIEZO
                print(f"[PIEZO Anomaly] Using default threshold: {self.threshold}")
        
        print(f"[PIEZO Anomaly] Final threshold: {self.threshold}")
        print(f"[PIEZO Anomaly]  Initialization complete")
    ...
    def _setup_logging(self):
        ...
        if self.log_format == "json":
            self.log_file = self.log_dir / f"piezo_anomalies_{today}.json"
            # Initialize with empty list if file doesn't exist
            if not self.log_file.exists():
                with open(self.log_file, 'w') as f:
                    json.dump([], f)
                print(f"[PIEZO Anomaly] Created new log file: {self.log_file}")
            else:
                print(f"[PIEZO Anomaly] Appending to existing log file: {self.log_file}")
        else:  # CSV
            self.log_file = self.log_dir / f"piezo_anomalies_{today}.csv"
    ...
    def _clean_old_logs(self):
        ...
        for log_file in self.log_dir.glob("piezo_anomalies_*.json"):
            try:
                # Extract date from filename: piezo_anomalies_YYYYMMDD.json
                ...
                if file_date < cutoff_date:
                    log_file.unlink()
                    deleted_count += 1
                    print(f"[PIEZO Anomaly] Deleted old log: {log_file.name}")
            except (ValueError, IndexError):
                # Skip files with unexpected naming format
                pass
        
        for log_file in self.log_dir.glob("piezo_anomalies_*.csv"):
            try:
                date_str = log_file.stem.split('_')[2]
                file_date = datetime.strptime(date_str, "%Y%m%d")
        ...
        if deleted_count > 0:
            print(f"[PIEZO Anomaly] Cleaned {deleted_count} old log file(s)")
    
    def preprocess_sample(self, data: np.ndarray) -> np.ndarray:
        """
        Preprocess PIEZO sample for model input
        
        Args:
            data: Raw PIEZO data (1D array)
    ...
    def detect_anomaly(self, data: np.ndarray) -> Dict:
        """
        Detect if PIEZO sample contains anomaly
        
        Args:
            data: Raw PIEZO data array
        ...
        if is_anomaly:
            ...
            if self.notification_callback:
                try:
                    self.notification_callback('piezo', result)
                except Exception as e:
                    print(f"[PIEZO Anomaly] Error in notification callback: {e}")
    ...
    def _log_anomaly(self, result: Dict, sample_data: np.ndarray):
        ...
        print(f"[PIEZO Anomaly]  DETECTED at {log_entry['time']} "
...
class PiezoAnomalyDetectionWorker:
    """
    Background worker that processes PIEZO data queue and detects anomalies
    """
    
    def __init__(self, detector: PiezoAnomalyDetector, 
                 window_size: int = 1000,
                 overlap_ratio: float = 0.0):
        """
        Args:
            detector: PiezoAnomalyDetector instance
        ...
        print(f"[PIEZO Anomaly] Worker config: window={window_size}, step={self.step_size}")
    ...
    def start(self):
        ...
        print("[PIEZO Anomaly] Worker started")
    ...
    def stop(self):
        ...
        print("[PIEZO Anomaly] Worker stopped")
    
    def add_data(self, piezo_samples: List[float]):
        """
        Add PIEZO samples to processing queue
        
        Args:
            piezo_samples: List of PIEZO values
        """
        try:
            self.data_queue.put_nowait(piezo_samples)
        except queue.Full:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                while len(self.buffer) >= self.window_size:
                    ...
                    piezo_data = np.array(window, dtype=np.float32)
                    
                    # Detect anomalies
                    self.detector.detect_anomaly(piezo_data)
                    
            except queue.Empty:
                continue
            except Exception as e:
                print(f"[PIEZO Anomaly] Worker error: {e}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("PIEZO Anomaly Detection - Test")
    print("=" * 60)
    
    try:
        # Initialize detector
        detector = PiezoAnomalyDetector(
            model_path="unified_piezo_anomaly_detector_quantized.tflite",
            log_format="json"
        )
        
        # Create worker with 75% overlap
        worker = PiezoAnomalyDetectionWorker(
            detector, 
            window_size=1000,
        ...
        # Simulate some data
        print("\nSimulating PIEZO data...")
        import time
        
        for i in range(10):
            # Generate fake normal PIEZO signal
